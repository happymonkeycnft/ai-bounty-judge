...
export default function ResultPage() {
  ...
  if (error && bounty && submission) return <AppShell><main className="narrow"><div className="empty-state"><strong>Review unavailable</strong><p>{error}</p><p>The live submission is {submission.finalized ? "finalized" : "not finalized"}. The controlled Bradbury review ended in a validator timeout; no APPROVED result was accepted.</p><a href={submission.primary_url} target="_blank" rel="noreferrer">{submission.primary_url} <ExternalLink size={14}/></a><p>{submission.notes}</p><code>{getContractAddress()}</code><a href={BRADBURY_EXPLORER} target="_blank" rel="noreferrer">Open Bradbury explorer <ExternalLink size={14}/></a><Link href={`/bounty/${id}`} className="button secondary">Return to bounty</Link></div></main></AppShell>;
